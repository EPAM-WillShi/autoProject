...
class AddLeaveEntitlement(Leave):
    ...
    def add_entitlement_for_multiple_employees(self, first_name, last_name, leave_type, leave_period,
        ...
        self.click(self.save_btn)
        self.sleep(5)
        assert self.get_element(self.flag).is_displayed
        emp_name = first_name + " " + last_name
        emp_ele = ('XPATH', self.list_name.format(emp_name))
        # self.wait_unit_el_present(emp_ele)
        assert self.get_element(emp_ele).is_displayed
